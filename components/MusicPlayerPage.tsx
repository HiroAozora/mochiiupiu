...
export default function MusicPlayerPage({ onBack }: MusicPlayerPageProps) {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-[#FFF9F6]">
      <AnimatePresence>
        ...
        {view === "playlist" && (
          <motion.div 
            ...
            className="absolute inset-0 z-10 w-full h-full flex flex-col bg-[#FFF9F6] overflow-hidden"
          >
        ...
        {view === "player" && currentTrack && (
          <motion.div 
            ...
            className="absolute inset-0 z-30 w-full h-[100dvh] text-[#5E4E46] overflow-y-auto scroll-smooth bg-[#FFF9F6]"
            style={{ willChange: "transform" }}
          >
            {/* Dynamic Ambient Background (Light theme) */}
            <div className="absolute top-0 left-0 right-0 h-[100dvh] pointer-events-none z-0 overflow-hidden">
               {currentTrack.coverUrl && (
                 <div 
                   className="absolute inset-0 bg-top bg-cover bg-no-repeat scale-150 blur-[60px] opacity-40" 
                   style={{ backgroundImage: `url(${currentTrack.coverUrl})` }} 
                 />
               )}
               <div className="absolute inset-0 bg-white/40" />
               <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#FFF9F6]/80 to-[#FFF9F6]" />
            </div>

            <div className="relative z-10 flex flex-col min-h-full pb-8 pt-6">
              
              {/* Header */}
              <div className="px-6 flex items-center justify-between mb-8">
                <button onClick={() => setView("playlist")} className="w-10 h-10 flex items-center justify-center text-[#5E4E46] active:scale-95 transition-transform drop-shadow-sm -ml-2">
                ...
                <div className="flex flex-col items-center drop-shadow-sm">
                  <span className="font-sans text-[10px] uppercase tracking-widest text-[#5E4E46]/70 font-bold">Sedang Diputar</span>
                  <span className="font-sans font-bold text-sm text-[#5E4E46]">Mochi's Mix</span>
                </div>
                <div className="w-10 h-10 flex items-center justify-center text-[#5E4E46] drop-shadow-sm -mr-2">
              ...
              {/* Large Album Cover */}
              <div className="px-8 mb-6 w-full max-w-[360px] mx-auto">
                 <div className="w-full aspect-square rounded-[24px] overflow-hidden shadow-[0_20px_40px_rgba(255,126,83,0.15)] bg-white border border-[#FFDFC6]/30">
                   {currentTrack.coverUrl ? (
                     <img src={currentTrack.coverUrl} alt="Cover" className="w-full h-full object-cover" />
                   ) : (
                     <div className="w-full h-full flex items-center justify-center text-[#FF7E53]/50">
                       <svg viewBox="0 0 24 24" fill="currentColor" className="w-20 h-20"><path d="M9 18V5l12-2v13" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" /></svg>
                     </div>
                   )}
                 </div>
              </div>

              {/* Canvas Scrolling Lyrics (Now placed BETWEEN cover and track info) */}
              <div 
                className="px-8 mb-6 h-[40px] overflow-hidden pointer-events-none relative z-10"
              ...
              >
                <motion.div 
                  ...
                  animate={{ y: activeLyricIndex !== -1 ? -(activeLyricIndex * 40) : 0 }}
                ...
                >
                  {lyrics.length > 0 && lyrics.map((line, i) => {
                    ...
                    return (
                      <div 
                        key={i} 
                        className={`h-[40px] flex items-center justify-start text-left font-fredoka transition-all duration-300 ${
                          isActive ? "text-[#5E4E46] font-bold text-[18px]" : 
                          "text-[#5E4E46]/40 text-[16px]"
                        }`}
                      >
              ...
              <div className="px-8 flex items-center justify-between mb-4">
                <div className="flex flex-col min-w-0 pr-4">
                  <h2 className="font-fredoka text-[24px] font-bold truncate text-[#5E4E46]">{currentTrack.title}</h2>
                  <p className="font-sans text-base text-[#5E4E46]/70 truncate">{currentTrack.artist}</p>
                </div>
                <button className="text-[#FF7E53] shrink-0 hover:scale-110 transition-transform">
              ...
              {/* Seekbar */}
              <div className="px-8 flex flex-col gap-2 mb-6">
              ...
              <div className="px-8 flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-8">
                  ...
                  <button onClick={togglePlay} className="w-[68px] h-[68px] rounded-full bg-[#FF7E53] text-white flex items-center justify-center hover:scale-105 shadow-lg shadow-[#FF7E53]/30 transition-transform">
              ...
              {/* THE LYRICS BOX (Mochi Light Theme) */}
              <div className="px-4 mt-2 mb-10 relative z-20">
                <div className="w-full bg-[#FFEFE6] rounded-[24px] p-6 shadow-[0_4px_20px_rgba(255,126,83,0.08)] border border-[#FFDFC6]/50 flex flex-col h-[65vh]">
                  {/* Lirik Header */}
        ...
        {view === "lyrics" && currentTrack && (
          <motion.div 
            ...
            transition={{ type: "spring", damping: 26, stiffness: 220 }}
            className="absolute inset-0 z-40 w-full h-full text-[#5E4E46] overflow-hidden bg-[#FFEFE6] flex flex-col"
            style={{ willChange: "transform" }}
          >
